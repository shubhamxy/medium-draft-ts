import {EditorState, genKey, Modifier, SelectionState, ContentState} from 'draft-js';
...
export function uploadHelper({getEditorState, setEditorState}: StateGetterSetter, files: Blob[], options: UploadHelperOptions, selection?: SelectionState) {
    ...
    if (options && options.uploadImage) {
        ...
        options.uploadImage(files[0]).then((data) => {
            ...
            setEditorState(EditorState.forceSelection(stateWithLoadedImage, oldSelection));
        });
    }
}

function replaceStack(oldStack: Stack<ContentState>, replaceableContent: ContentState, newContent: ContentState): Stack<ContentState> {
